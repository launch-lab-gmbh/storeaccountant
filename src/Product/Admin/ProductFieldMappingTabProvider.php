...
/**
 * Provides the field mapping tab for WooCommerce product export configurations.
 */
final readonly class ProductFieldMappingTabProvider implements ExportConfigurationTabProviderInterface, HookRegistrarInterface {
	public const PROVIDER_ID = 'product_field_mapping';
	...
	public function register(): void {
		...
		add_action( 'admin_post_storeaccountant_save_product_field_mapping', [ $this, 'handle_save' ] );
	}
	...
	public function supports( WP_Post $configuration ): bool {
		...
		return ProductExportAdapter::ADAPTER_ID === $export_adapter;
	}
	...
	public function render( string $tab, WP_Post $configuration, bool $read_only = false ): void {
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			<input type="hidden" name="action" value="storeaccountant_save_product_field_mapping" />
			<input type="hidden" name="storeaccountant_export_configuration_id" value="<?php echo esc_attr( (string) $configuration->ID ); ?>" />
			<?php wp_nonce_field( 'storeaccountant_save_product_field_mapping', 'storeaccountant_field_mapping_nonce' ); ?>
	...
	public function handle_save(): void {
		...
		check_admin_referer( 'storeaccountant_save_product_field_mapping', 'storeaccountant_field_mapping_nonce' );
	...
	/**
	 * Logs a diagnostic incident for product field mapping save failures.
	 */
	...
	private function get_field_description( Field $field ): string {
		if ( $this->is_product_meta_field( $field ) ) {
			return sprintf(
				/* translators: %s: WooCommerce product meta key. */
				__( 'Custom product field: %s', 'storeaccountant' ),
				(string) $field->options[ ProductMetaFieldProvider::OPTION_META_KEY ]
			);
		}

		return match ( $field->id ) {
			'product_id' => __( 'product_id', 'storeaccountant' ),
			'parent_product_id' => __( 'parent_product_id', 'storeaccountant' ),
			'name' => __( 'name', 'storeaccountant' ),
			'slug' => __( 'slug', 'storeaccountant' ),
			'sku' => __( 'sku', 'storeaccountant' ),
			'parent_sku' => __( 'parent_sku', 'storeaccountant' ),
			'product_type' => __( 'product_type', 'storeaccountant' ),
			'status' => __( 'status', 'storeaccountant' ),
			'catalog_visibility' => __( 'catalog_visibility', 'storeaccountant' ),
			'date_created' => __( 'date_created', 'storeaccountant' ),
			'date_modified' => __( 'date_modified', 'storeaccountant' ),
			'permalink' => __( 'permalink', 'storeaccountant' ),
			'menu_order' => __( 'menu_order', 'storeaccountant' ),
			'total_sales' => __( 'total_sales', 'storeaccountant' ),
			'average_rating' => __( 'average_rating', 'storeaccountant' ),
			'review_count' => __( 'review_count', 'storeaccountant' ),
			'featured' => __( 'featured', 'storeaccountant' ),
			'virtual' => __( 'virtual', 'storeaccountant' ),
			'downloadable' => __( 'downloadable', 'storeaccountant' ),
			'manage_stock' => __( 'manage_stock', 'storeaccountant' ),
			'sold_individually' => __( 'sold_individually', 'storeaccountant' ),
			'regular_price' => __( 'regular_price', 'storeaccountant' ),
			'sale_price' => __( 'sale_price', 'storeaccountant' ),
			'price' => __( 'price', 'storeaccountant' ),
			'stock_quantity' => __( 'stock_quantity', 'storeaccountant' ),
			'weight' => __( 'weight', 'storeaccountant' ),
			'length' => __( 'length', 'storeaccountant' ),
			'width' => __( 'width', 'storeaccountant' ),
			'height' => __( 'height', 'storeaccountant' ),
			'tax_status' => __( 'tax_status', 'storeaccountant' ),
			'tax_class' => __( 'tax_class', 'storeaccountant' ),
			'stock_status' => __( 'stock_status', 'storeaccountant' ),
			'backorders' => __( 'backorders', 'storeaccountant' ),
			'categories' => __( 'categories', 'storeaccountant' ),
			'tags' => __( 'tags', 'storeaccountant' ),
			'attributes' => __( 'attributes', 'storeaccountant' ),
			'default_attributes' => __( 'default_attributes', 'storeaccountant' ),
			'variation_attributes' => __( 'variation_attributes', 'storeaccountant' ),
			'description' => __( 'description', 'storeaccountant' ),
			'short_description' => __( 'short_description', 'storeaccountant' ),
			default => '',
		};
	}

	/**
	 * Checks whether a field represents WooCommerce product meta.
	 *
	 * @param Field $field Export field.
	 */
	private function is_product_meta_field( Field $field ): bool {
		return str_starts_with( $field->id, ProductMetaFieldProvider::FIELD_ID_PREFIX )
			&& isset( $field->options[ ProductMetaFieldProvider::OPTION_META_KEY ] )
			&& is_scalar( $field->options[ ProductMetaFieldProvider::OPTION_META_KEY ] );
	}
	...
	private function get_context( int $configuration_id ): ExportContext {
		$products = $this->get_configuration_products( $configuration_id );
	...
	/**
	 * Gets products covered by the current configuration.
	 *
	 * @param int $configuration_id Configuration post ID.
	 *
	 * @return array<int, mixed>
	 */
	private function get_configuration_products( int $configuration_id ): array {
