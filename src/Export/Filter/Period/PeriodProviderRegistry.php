<?php
/**
 * StoreAccountant
 * Export plugin for WooCommerce accounting workflows.
 *
 * @copyright   LaunchLab GmbH
 * @author      thomas.baier@launch-lab.de
 * @author-uri  https://launch-lab.de
 * @license     GPL-3.0-or-later
 */

declare(strict_types=1);

namespace StoreAccountant\Export\Filter\Period;

use StoreAccountant\Contract\RegistryItemInterface;
use StoreAccountant\Export\Filter\Period\Contract\PeriodProviderInterface;
use StoreAccountant\Registry;

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

/**
 * Collects period providers used by export filters.
 */
final readonly class PeriodProviderRegistry extends Registry {
	/**
	 * {@inheritDoc}
	 */
	protected function get_hook_name(): string {
		return 'storeaccountant_export_filter_period_provider';
	}

	/**
	 * {@inheritDoc}
	 *
	 * @return class-string<RegistryItemInterface>
	 */
	protected function get_type(): string {
		return PeriodProviderInterface::class;
	}
}
