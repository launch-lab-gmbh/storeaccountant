...
final readonly class AccountingHeaderBar {
	...
	public function render_overview_actions(): void {
		...
		<?php $this->render_list_tabs( ExportPostType::POST_TYPE ); ?>
	...
	public function render_configuration_actions(): void {
		?>
		<?php $this->render_global_actions(); ?>
			<?php $this->render_list_tabs( ExportConfigurationPostType::POST_TYPE ); ?>
	 ...
	 *
	 * @param string $active_post_type Active post type.
	 */
	private function render_list_tabs( string $active_post_type ): void {
		$tabs = [
			ExportPostType::POST_TYPE              => [
				'label' => __( 'Exports', 'storeaccountant' ),
				'url'   => $this->get_overview_url(),
			],
			ExportConfigurationPostType::POST_TYPE => [
				'label' => __( 'Export Configurations', 'storeaccountant' ),
				'url'   => $this->get_configuration_overview_url(),
			],
		];

		if ( ! $this->permissions->can( PermissionActionIds::EXPORT_LIST ) ) {
			unset( $tabs[ ExportPostType::POST_TYPE ] );
		}

		if ( ! $this->permissions->can( PermissionActionIds::CONFIGURATION_LIST ) ) {
			unset( $tabs[ ExportConfigurationPostType::POST_TYPE ] );
		}
		?>
		<nav class="nav-tab-wrapper storeaccountant-list-tabs" aria-label="<?php echo esc_attr__( 'Accounting navigation', 'storeaccountant' ); ?>">
			<?php foreach ( $tabs as $post_type => $tab ) : ?>
				<a class="<?php echo esc_attr( $this->get_tab_class( $post_type, $active_post_type ) ); ?>" href="<?php echo esc_url( $tab['url'] ); ?>">
	 ...
	 *
	 * @param string $post_type        Tab post type.
	 * @param string $active_post_type Active post type.
	 */
	private function get_tab_class( string $post_type, string $active_post_type ): string {
		$classes = [ 'nav-tab' ];

		if ( $post_type === $active_post_type ) {
