...
/**
 * Tests the product date filter field provider.
 */
final class ProductDateFilterFieldProviderTest extends TestCase {
	...
	public function test_get_id_and_supports_are_stable(): void {
		...
		self::assertSame( ProductDateFilter::FILTER_ID, $provider->get_id() );
	...
	public function test_get_selection_from_request_delegates_to_period_field_provider(): void {
		...
		self::assertSame( ProductDateFilter::FILTER_ID, $selection->filter_id );
		self::assertSame( ProductDateFilter::FIELD_DATE_CREATED, $selection->settings['date_field'] );
	...
	private function provider(): ProductDateFilterFieldProvider {
		return new ProductDateFilterFieldProvider( new MonthYearExportPeriodFieldProvider() );
	}
